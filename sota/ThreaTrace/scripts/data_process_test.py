...
# Try to import weight function and switch from config file, if not exists use default function
try:
	from edge_weight_config import get_edge_weight, ENABLE_WEIGHT
except ImportError:
	# If config file does not exist, use default weight function and switch
	ENABLE_WEIGHT = False
	def get_edge_weight(edge_type_str):
		"""Default weight function, all edge types have weight 1.0"""
		return 1.0
...
def MyDatasetA(path, model):
	...
	feature_map = {}
	feature_map_reverse = {}  # Reverse mapping: from numeric ID to edge type string
	for i in f_feature:
		temp = i.strip('\n').split('\t')
	...
	edge_type_str_list = [] if ENABLE_WEIGHT else None  # Store original edge type strings for weight calculation (only when weights are enabled)
	adj = {}
	adj2 = {}
	data_thre = 1000000
	...
	for i in range(1):
		...
		for line in f:
			...
			edge_type_str = temp[4]  # Save original edge type string
			temp[4] = feature_map[edge_type_str]
			edge_s.append(temp[0])
			edge_e.append(temp[2])
			if ENABLE_WEIGHT:
				edge_type_str_list.append(edge_type_str)  # Save original edge type string
	...
	# Decide whether to calculate and apply weights based on switch
	if ENABLE_WEIGHT:
		# First calculate weight for each edge based on edge type, simultaneously count weight sum and degree of adjacent edges for each node
		...
		for (s, e), edge_type_str in zip(zip(edge_s, edge_e), edge_type_str_list):
			...
			# Count weight sum and degree of both end nodes
			node_weight_sum[s] += weight
			node_weight_sum[e] += weight
			node_weight_deg[s] += 1
			node_weight_deg[e] += 1
		# Calculate node weights: use average of adjacent edge weights (nodes without edges keep 1.0)
		...
		# When not using edge weights, don't pass any weights
